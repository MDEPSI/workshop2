 ...
 */
	...
	{
		$req = "select equipe.id as ID, apprenant.nom as NOM, apprenant.prenom as PRENOM, equipe.libelle as LIBELLE, equipe.nbJetons as NBJETONS from apprenant right join equipe on apprenant.id_equipe=equipe.id";
		$res = PdoWorkshop2::$monPdo->query($req);
	...
	{
		$req = "select projet.id as ID, projet.libelle as LIBELLE, projet.nbJetons as NBJETONS, responsable.nom as NOM, responsable.prenom as PRENOM from projet join responsable on projet.id_responsable=responsable.id";
		$res = PdoWorkshop2::$monPdo->query($req);
